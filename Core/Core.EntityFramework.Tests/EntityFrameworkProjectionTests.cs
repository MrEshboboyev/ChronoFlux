    ...
    : IClassFixture<EfCorePostgresContainerFixture<TestDbContext>>
{
    ...
    public async Task Applies_Works_Separately()
    {
        ...
        await projection.HandleAsync([EventEnvelope.From(new Opened(cartId, clientId))], CancellationToken.None);
        ...
        await projection.HandleAsync([EventEnvelope.From(new ProductItemAdded(cartId, 2))], CancellationToken.None);
        await context.SaveChangesAsync();
        ...
        await projection.HandleAsync([EventEnvelope.From(new Cancelled(cartId))], CancellationToken.None);
        await context.SaveChangesAsync();

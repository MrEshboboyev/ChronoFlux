    ...
    )
    {
        ...
        try
        {
            foreach (var @event in eventRange.Events)
            {
                ...
                await activityScope.RunAsync($"{nameof(MartenEventPublisher)}/{nameof(ProcessEventsAsync)}",
                    async (_, ct) =>
                    {
                        ...
                        await eventBus.PublishAsync(EventEnvelope.From(@event.Data, eventMetadata), ct)

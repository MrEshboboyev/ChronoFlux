...
using Xunit;
using GuestCheckoutFailed = HotelManagement.Sagas.GuestStayAccounts.GuestCheckoutFailed;
...
public class GroupCheckoutSagaTests
{
    ...
    [Fact]
    public async Task GroupCheckoutInitiated_ShouldSchedule_CheckOutGuestForEachGuestAndRecordGuestCheckoutsInitiation()
    {
        // Given
        ...
        // When
        await saga.HandleAsync(@event, ct);
...
internal class AsyncCommandBusStub: IAsyncCommandBus
{
    ...
    public Task ScheduleAsync<TCommand>(TCommand command, CancellationToken ct = default) where TCommand : notnull
    {

        ...
        IEventHandler<GuestStayAccounts.GuestCheckoutFailed>
{
    ...
    public Task HandleAsync(GroupCheckoutInitiated @event, CancellationToken ct)
    {
    ...
    public Task HandleAsync(Choreography.GuestStayAccounts.GuestCheckoutFailed @event, CancellationToken ct)
    {
        if (!@event.GroupCheckOutId.HasValue)
            return Task.CompletedTask;

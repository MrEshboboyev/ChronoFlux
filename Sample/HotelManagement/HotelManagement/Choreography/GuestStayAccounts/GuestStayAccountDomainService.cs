...
{
    public Task HandleAsync(CheckInGuest command, CancellationToken ct) =>
    ...
    public Task HandleAsync(RecordCharge command, CancellationToken ct) =>
    ...
    public Task HandleAsync(RecordPayment command,CancellationToken ct) =>
        documentSession.GetAndUpdate(
            command.GuestStayId,
            command.ExpectedVersion,
    ...
    public Task HandleAsync(CheckOutGuest command, CancellationToken ct) =>
        documentSession.GetAndUpdate(
            command.GuestStayId,

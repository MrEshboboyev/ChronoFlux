...
{
    public Task HandleAsync(InitiateGroupCheckout command, CancellationToken ct) =>
    ...
    public Task HandleAsync(RecordGuestCheckoutsInitiation command, CancellationToken ct) =>
    ...
    public Task HandleAsync(RecordGuestCheckoutCompletion command, CancellationToken ct) =>
        documentSession.GetAndUpdate(
            command.GuestStayId,
    ...
    public Task HandleAsync(RecordGuestCheckoutFailure command, CancellationToken ct) =>
        documentSession.GetAndUpdate(
            command.GuestStayId,
